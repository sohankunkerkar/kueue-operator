...
# kannon92/kueue-operator-bundle:$VERSION and kannon92/kueue-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= kannon92/kueue-operator
...
.PHONY: deploy-operator-on-kind
deploy-operator-on-kind:
	export KIND=$(KIND) KUBECTL=$(KUBECTL) IMG=$(IMG) && \
		hack/deploy-operator-on-kind.sh
